...
if __name__ == '__main__':
    ...
    for i in range(0,10):
        ...
        parser.add_argument('--seed', type=int, default=seeds[i])
                # dataset
        ...
        parser.add_argument('--runNo', type=int, default=i) # specified runNO
        parser.add_argument('--K', type=int, default=0) # specified K
        ...
        parser.add_argument('--output_path', type=str, metavar='PATH',
                                    default=osp.join(working_dir, 'data/outputs'))

        E_cluster_discrete_set,fairness_error_set,avg_balance_set,min_balance_set=main(parser.parse_args())
        print("Len of E discrete "+str(len(E_cluster_discrete_set)))
        E_cluster_discrete_run.append(np.mean(E_cluster_discrete_set))
        fairness_error_run.append(np.mean(fairness_error_set))
        avg_balance_run.append(np.mean(avg_balance_set))
        min_balance_run.append(np.mean(min_balance_set))


    print("Clustering Objective Discrete over runs "+str(E_cluster_discrete_run))
    print("Fairness Error over runs "+str(fairness_error_run))
    print("Average Balance over runs "+str(avg_balance_run))
    print("Average Balance over runs "+str(min_balance_run))
    print("*"*20)
    print("Mean Clustering Objective Discrete over runs "+str(np.mean(E_cluster_discrete_run)))
    print("Mean Fairness Error over runs "+str(np.mean(fairness_error_run)))
    print("Mean Average Balance over runs "+str(np.mean(avg_balance_run)))
    print("Mean Min Balance over runs "+str(np.mean(min_balance_run)))
      
    print("*"*20)
    print("Std Dev. Clustering Objective Discrete over runs "+str(np.std(E_cluster_discrete_run)))
    print("Std Dev. Fairness Error over runs "+str(np.std(fairness_error_run)))
    print("Std Dev. Average Balance over runs "+str(np.std(avg_balance_run)))
    print("Std Dev. Min Balance over runs "+str(np.std(min_balance_run)))
    print("*"*20)
